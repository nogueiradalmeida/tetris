...
/*** /	fama("ZE",rand()%10000); /**/
	...
	{
		//setviewport(0, 0, getmaxx() , getmaxy(), CLIP_ON);
		botao(tx4, 302, tx4+300, 332, 7, 1, " Tente sem trapaca!! ",1);
		getch();
		exit(1);
	}
	...
	//CALCADA DA FAMA !!!
	cleardevice();
	janela(getmaxx()/2-150, 100, getmaxx()/2+150, 210, 7, 9, 1, "JJUNIOR TETRIS (R) ",15);
...
void zeraTela (char **tela, int modo)
{
	...
	for(i=0;i<tx4/QUAD;i++)
		...
		{
			tela[i][j]='0';
			quad(i*QUAD,j*QUAD,9,1,15,modo);
			//printf ("%c",tela[i][j]);
		}
...
int menu (void)
{
	...
	while (op!=1)
	{
		janela(getmaxx()/2-100, 102, getmaxx()/2+100, 412, 7, 9, 1, "MENU",15);
		botao(getmaxx()/2-50, 152, getmaxx()/2+50, 182, 7, 1, "1. INICIAR",15);
		botao(getmaxx()/2-50, 202, getmaxx()/2+50, 232, 7, 1, "2. MODO   ",15);
		botao(getmaxx()/2-50, 252, getmaxx()/2+50, 282, 7, 1, "3. NIVEL  ",15);
		botao(getmaxx()/2-50, 302, getmaxx()/2+50, 332, 7, 1, "4. TAMANHO",15);
		botao(getmaxx()/2-50, 352, getmaxx()/2+50, 382, 7, 1, "5. SAIR   ",15);

		botao(getmaxx()/2-250, 422, getmaxx()/2+250, 452, 7, 1, "Escolha o numero correspondente a opcao",BLUE);
		...
		switch (op)
		{
			...
			case 2:
				janela(getmaxx()/2+120, 102, getmaxx()/2+320, 262, 7, 9, 1, "MODELO DAS PECA",15);
			...
			case 3:
				janela(getmaxx()/2+120, 102, getmaxx()/2+320, 262, 7, 9, 1, "NIVEL DE DIFICULDADE",15);
			...
			case 4:
				janela(getmaxx()/2+120, 102, getmaxx()/2+320, 262, 7, 9, 1, "TAMANHO DAS PECAS",15);
			...
			case 5:
				botao(getmaxx()/2-250, 422, getmaxx()/2+250, 452, 7, 1, "Vai sair do jogo pela-saco? (S/N)",BLUE);
				op=toupper(getch());
				if(op==13 || op=='S' || op=='Y')
					{
					botao(getmaxx()/2-250, 422, getmaxx()/2+250, 452, 7, 1, "Maior mancada ai!!",BLUE);
					getch();
					//closegraph();
					exit(1);
					}
				botao(getmaxx()/2-250, 422, getmaxx()/2+250, 452, 7, 1, "",BLUE);
				setcolor(BLACK);
				rectangle(tx4, 302, tx4+300, 332);
				break;
		}
