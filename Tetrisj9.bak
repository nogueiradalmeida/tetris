...
int jogo()
{                              /**** quebraLoop1=0/**/
	int obstaculo=0, quebraLoop1=0,ultimo[2], pula=0, pos=1, paratudo;
...
/***///COMECA AKI O LOOP PARA JOGAR NOVAMENTE
do{
...
int fama(char *nome, int pontos)
{
	...
	*/
	...
	if (contador==10 && pontos>menorPontuacao && pontos!=NULL)
	{
		//contador-=1;
	...
	else if (contador<10 && pontos!=NULL)
	{
		contador++;
		strcpy(famoso[contador].nome, nome);
		famoso[contador].pontos = pontos;
...
void saindo(void)
{
	...
	for(i=0;i<=contador;i++)
	{
		fwrite (&famoso[i],   sizeof(Famoso), 1, pa);
	}
